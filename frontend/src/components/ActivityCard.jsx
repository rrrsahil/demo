  ...
  return (
    <div className={`activity-card ${selected ? 'selected' : ''}`}>
      <div className="activity-card-header">
        <span className="activity-emoji">{CATEGORY_ICONS[activity.category] || '📌'}</span>
      ...
      <div className="activity-meta">
        <span><i className="fas fa-clock" /> {activity.duration}</span>
        <span><i className="fas fa-star" style={{ color: '#f59e0b' }} /> {activity.rating?.toFixed(1)}</span>
      </div>
      <div className="activity-footer">
        ...
        {onAdd && (
          ...
          >
            {selected ? <><i className="fas fa-check" /> Added</> : <><i className="fas fa-plus" /> Add</>}
          </button>
        )}
      </div>
    </div>
