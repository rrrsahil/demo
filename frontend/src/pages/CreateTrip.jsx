...
const CreateTrip = () => {
  ...
  const [form, setForm] = useState({
    tripName: '', description: '', startDate: '', endDate: '', isPublic: false,
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await api.post('/trips/create', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      navigate(`/trips/${res.data.trip._id}/itinerary`)
    } catch (err) {
  ...
  return (
    <div className="create-trip-wrap">
      <div style={{ marginBottom: '24px' }}>
        <h1 className="page-title"><i className="fas fa-plus-circle" style={{ color: 'var(--primary)', marginRight: '10px' }} />Create New Trip</h1>
        <p className="page-subtitle">Plan your next adventure step by step</p>
      </div>
      ...
      <div className="create-trip-card">
        ...
        {step === 1 && (
          <div>
            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
              Add the cities you'll be visiting on this trip.
            </p>
            ...
            {destinations.length === 0 && (
              <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '8px' }}>
                <i className="fas fa-info-circle" /> You can also skip this and add destinations later.
              </p>
            )}
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
              Add a cover photo for your trip (optional).
            </p>
        ...
        <div className="form-actions">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button className="btn btn-primary" onClick={handleSubmit} disabled={loading}>
              {loading ? <><i className="fas fa-circle-notch fa-spin" /> Creating...</> : <><i className="fas fa-rocket" /> Create Trip</>}
            </button>
          )}
        </div>
      </div>
