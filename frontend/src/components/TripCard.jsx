  ...
  return (
    <div className="trip-card">
      ...
      <div className="trip-card-img">
        ...
        <span className="trip-card-status" style={{ background: status.bg, color: status.color }}>
          {trip.status}
        </span>
      </div>
      ...
      <div className="trip-card-body">
        ...
        {trip.destinations?.length > 0 && (
          <div className="trip-card-destinations">
            <i className="fas fa-location-dot" style={{ color: 'var(--primary)', fontSize: '12px' }} />
        ...
        <div className="trip-card-actions">
          <Link to={`/trips/${trip._id}/itinerary`} className="btn btn-primary btn-sm">
            <i className="fas fa-map" /> Itinerary
          </Link>
          <Link to={`/trips/${trip._id}/budget`} className="btn btn-secondary btn-sm">
            <i className="fas fa-wallet" /> Budget
          </Link>
          {onDelete && (
            <button className="btn btn-ghost btn-sm" onClick={() => onDelete(trip._id)}
              title="Delete trip" style={{ color: 'var(--danger)' }}>
