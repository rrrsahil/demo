...
const BudgetBreakdown = () => {
  ...
  const avgPerDay = calculateAveragePerDay(
    total,
    trip?.startDate,
    trip?.endDate,
  );

  const highestExpense = getHighestExpenseCategory(budget);

  const budgetAlert = getBudgetAlert(avgPerDay);
    ...
    <div>
      ...
      {saved && (
        <div className="alert alert-success">
          <i className="fas fa-circle-check" /> Budget saved successfully!
        </div>
      )}
      ...
      <div className="budget-layout">
        ...
        <div>
          ...
          >
            <div className="budget-analytics-card">
              <p className="budget-analytics-label">Average Per Day</p>

              <h3 className="budget-analytics-value">
                {formatCurrency(avgPerDay, budget.currency)}
              </h3>
            </div>

            <div className="budget-analytics-card">
              <p className="budget-analytics-label">Highest Expense</p>

              <h3 className="budget-analytics-value">
                {highestExpense?.label || "N/A"}
              </h3>
            </div>
          </div>

          {/* Budget Alert */}
          <div className={`budget-alert ${budgetAlert.type}`}>
          ...
          <div className="budget-form-card">
            ...
            >
              Fill in estimated costs per category
            </p>

            {BUDGET_FIELDS.map((f) => (
              <div className="budget-input-row" key={f.key}>
                <div
                  className="budget-input-icon"
            ...
            <div style={{ marginTop: "16px" }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setBudget((p) => ({ ...p, currency: e.target.value }))
                }
              >
            ...
            >
              {saving ? (
                ...
                <>
                  <i className="fas fa-floppy-disk" /> Save Budget
                </>
              )}
            </button>
          </div>
        ...
        <div>
          ...
          {/* Breakdown bars */}
          {total > 0 && (
            <div className="budget-form-card category-share-card">
              <h3
                style={{
                  fontSize: "15px",
                  fontWeight: "600",
